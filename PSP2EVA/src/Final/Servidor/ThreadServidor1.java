...
public class ThreadServidor1 extends Thread {
    ...
    public void run() {
        try {
            while (true) {
                ...
                //Esta es la orden recibida separada en una tablaZ
            ...
            try {
                ...
                ShareDataServidor1 shareData = (ShareDataServidor1) in.readObject();
                ArrayList<String[]> data = shareData.getInformacion();
                if(!sonIguales(data, this.baseDeDatos.getInformacion()))this.baseDeDatos.setInformacion(data);
                // Comprobar si ha cambiado el arrayList y si eso, hacer un set
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public boolean sonIguales(ArrayList<String[]> replica, ArrayList<String[]> original) {
        if (replica.size() != original.size()) {
            return false;
        }
        for (int i = 0; i < replica.size(); i++) {
            if (!Arrays.equals(replica.get(i), original.get(i))) {
                return false;
            }
        }
        return true;
